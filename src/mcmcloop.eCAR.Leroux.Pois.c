...
* E = nobs x 1 vector containing population values (i.e., population size in each county)
...
* eta = nout x ncov scratch vector that holds sig2 MCMC iterates
*****************************************************************************************/


void mcmcloop_leroux_pois(int *draws, int *burn, int *thin, int *nobs, double *y, double *x, double *E,
			                double *evals,  double *evecs, double *W,double *C, int *ncov,
			                double *modelPriors, double *MHsd, int *verbose,
...
//  RprintVecAsMat("evals", evals, 1, *nobs);
  ...
  double lamx_iter=0.7, lamz_iter=0.9, tau_iter=0.4375;
  ...
  double llo, lln, llr, uu, ld, ldeto, ldetn, to, tn, qf1, qf2, qf3, qf4, qf5;
  ...
  double m=modelPriors[0], s2=modelPriors[1]; // prior values for (alpha, beta) ~ N(m, s2I)
	...
	if(*verbose){
    Rprintf("Prior values are: m=%f, s2=%f, alamx=%f, blamx=%f, \n alamz=%f, blamz=%f, asig=%f, bsig=%f,  atau=%f, btau=%f, \n asigx=%f, bsigx=%f, mb0=%f, s2b0=%f, me=%f, s2e=%f\n",
